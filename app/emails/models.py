...
class ParsedEmail(BaseModel):
    ...
    currency: str | None = Field(
        default=None, description="Currency code (e.g., NGN, USD)"
    )
    ...
    recipient_name: str | None = Field(
        default=None, description="Transaction recipient name"
    )
    reference: str | None = Field(
        default=None, description="Transaction reference number"
    )
    account_number: str | None = Field(default=None, description="Account number")
    email_timestamp: datetime | None = Field(
        default=None, description="Transaction timestamp from email"
    )
    ...
    confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Parsing confidence score"
    )
...
class FilterResult(BaseModel):
    ...
    matched_whitelist: bool = Field(
        default=False, description="Matched sender whitelist"
    )
    matched_keywords: list[str] = Field(
        default_factory=list, description="Matched keywords"
    )
    matched_blacklist: list[str] = Field(
        default_factory=list, description="Matched blacklist patterns"
    )
...
class RegexExtractionResult(BaseModel):
    ...
    patterns_matched: dict[str, str] = Field(
        default_factory=dict, description="Patterns that matched"
    )
