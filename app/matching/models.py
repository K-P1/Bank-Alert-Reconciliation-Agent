...
class RuleScore(BaseModel):
    ...
    details: dict = Field(default_factory=dict, description="Additional rule-specific details")
...
class MatchCandidate(BaseModel):
    ...
    # Matching scores
    rule_scores: list[RuleScore] = Field(default_factory=list, description="Individual rule scores")
    total_score: float = Field(default=0.0, ge=0.0, le=1.0, description="Total confidence score")
    ...
    def add_rule_score(self, rule_name: str, score: float, weight: float, details: dict | None = None) -> None:
...
class MatchResult(BaseModel):
    ...
    match_status: Literal["auto_matched", "needs_review", "rejected", "no_candidates"] = Field(
        ..., description="Match decision status"
    )
    
    # Best match (if any)
    best_candidate: MatchCandidate | None = Field(default=None, description="Best matching candidate")
    
    # Alternative matches
    alternative_candidates: list[MatchCandidate] = Field(
        default_factory=list,
        description="Other potential matches"
    )
    
    # Matching metadata
    total_candidates_retrieved: int = Field(default=0, description="Total candidates retrieved")
    total_candidates_scored: int = Field(default=0, description="Total candidates scored")
    ...
    # Processing notes
    notes: list[str] = Field(default_factory=list, description="Processing notes and warnings")
    ...
    def get_match_summary(self) -> dict:
        ...
        return {
            ...
            "transaction_id": self.best_candidate.transaction_id if self.best_candidate else None,
            "external_transaction_id": self.best_candidate.external_transaction_id if self.best_candidate else None,
...
class BatchMatchResult(BaseModel):
    ...
    results: list[MatchResult] = Field(default_factory=list, description="Individual match results")
    ...
    # Performance metrics
    average_confidence: float = Field(default=0.0, description="Average confidence of matches")
    average_candidates_per_email: float = Field(default=0.0, description="Average candidates retrieved")
    ...
    def finalize(self) -> None:
        ...
        if self.total_emails > 0:
            ...
            self.average_confidence = sum(matched_confidences) / len(matched_confidences) if matched_confidences else 0.0
    ...
    def get_summary(self) -> dict:
        ...
        return {
            ...
            "match_rate": self.total_matched / self.total_emails if self.total_emails > 0 else 0.0,
