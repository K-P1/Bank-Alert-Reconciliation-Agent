...
class MatchingRules:
    ...
    def exact_reference_match(
        ...
        details = {
            ...
            "transaction_reference": (
                transaction.reference.original if transaction.reference else None
            ),
            "match_type": "none",
        }
    ...
    def fuzzy_reference_match(
        ...
        similarity = self.fuzzy_matcher.comprehensive_similarity(
            email.reference.cleaned, transaction.reference.cleaned
        )
    ...
    def token_reference_match(
        ...
        details: dict[str, Any] = {
            ...
            "transaction_tokens": (
                transaction.reference.tokens if transaction.reference else []
            ),
        }
        ...
        score = self.fuzzy_matcher.match_tokens(
            email.reference.tokens, transaction.reference.tokens
        )

        details["token_match_score"] = score
        details["common_tokens"] = list(
            set(email.reference.tokens) & set(transaction.reference.tokens)
        )
    ...
    def account_match(
        ...
        email_last4 = (
            email.account_number[-4:]
            if len(email.account_number) >= 4
            else email.account_number
        )
        txn_last4 = (
            transaction.account_ref[-4:]
            if len(transaction.account_ref) >= 4
            else transaction.account_ref
        )
        ...
        similarity = self.fuzzy_matcher.simple_ratio(
            email.account_number, transaction.account_ref
        )
    ...
    def composite_key_match(
        ...
        details: dict[str, Any] = {
            "email_key": (
                email.composite_key.to_string() if email.composite_key else None
            ),
            "transaction_key": (
                transaction.composite_key.to_string()
                if transaction.composite_key
                else None
            ),
        }
        ...
        if email_tokens and txn_tokens:
            token_overlap = len(email_tokens & txn_tokens) / max(
                len(email_tokens), len(txn_tokens)
            )
        ...
        # Check account last4
        if (
            email.composite_key.account_last4
            and transaction.composite_key.account_last4
        ):
            if (
                email.composite_key.account_last4
                == transaction.composite_key.account_last4
            ):
                partial_matches += 1
    ...
    def bank_match(
        ...
        details: dict[str, Any] = {
            ...
            "transaction_bank": (
                transaction.enrichment.bank_code if transaction.enrichment else None
            ),
        }
        ...
        if email.enrichment.bank_code == transaction.enrichment.bank_code:
            ...
            avg_confidence = (
                email.enrichment.enrichment_confidence
                + transaction.enrichment.enrichment_confidence
            ) / 2
