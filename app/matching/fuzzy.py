...
class FuzzyMatcher:
    ...
    def best_match(
        self, query: str, choices: list[str], scorer: str = "token_sort"
    ...
    def match_tokens(self, tokens1: list[str], tokens2: list[str]) -> float:
        ...
        # Filter short tokens
        tokens1_filtered = [
            t for t in tokens1 if len(t) >= self.config.min_token_length
        ]
        tokens2_filtered = [
            t for t in tokens2 if len(t) >= self.config.min_token_length
        ]
        ...
        for token1 in tokens1_filtered:
            best_match, score = self.best_match(
                token1, tokens2_filtered, scorer="simple"
            )
...
def quick_ratio(
    str1: str | None, str2: str | None, min_similarity: float = 0.6
) -> float:
    """
