...
from app.matching.models import MatchResult, BatchMatchResult, MatchCandidate
...
class MatchingMetrics(BaseModel):
        ...
        if result.matched and result.best_candidate:
            ...
            for rule_score in result.best_candidate.rule_scores:
                ...
                if rule_name not in self.rule_contributions:
                    self.rule_contributions[rule_name] = RuleContribution(rule_name=rule_name)
        ...
        summary = {
            ...
            "matching": {
                ...
                "match_rate": (
                    ...
                    if self.total_emails_processed > 0 else 0.0
                ),
            },
            "candidates": {
    ...
    def print_summary(self) -> None:
        ...
        print(f"  ✓ Matched: {self.total_matches} ({summary['matching']['match_rate']:.1%})")
        ...
        if dist:
            print(f"  Very High (≥0.90): {dist['counts']['very_high']} ({dist['very_high_pct']:.1%})")
            print(f"  High (0.80-0.89): {dist['counts']['high']} ({dist['high_pct']:.1%})")
            print(f"  Medium (0.60-0.79): {dist['counts']['medium']} ({dist['medium_pct']:.1%})")
            print(f"  Low (0.40-0.59): {dist['counts']['low']} ({dist['low_pct']:.1%})")
            print(f"  Very Low (<0.40): {dist['counts']['very_low']} ({dist['very_low_pct']:.1%})")
            
        print(f"\nTop Contributing Rules:")
        sorted_rules = sorted(
            self.rule_contributions.items(),
