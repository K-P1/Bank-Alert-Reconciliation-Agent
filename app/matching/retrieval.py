...
from app.matching.config import MatchingConfig, CandidateRetrievalConfig
...
class CandidateRetriever:
    ...
    async def get_candidates_for_email(
        ...
        if email.amount is None:
            logger.warning(f"Email {email.message_id} has no amount, cannot retrieve candidates")
            return []
    ...
    def _post_filter_candidates(
        self,
        email: NormalizedEmail,
        candidates: list[NormalizedTransaction]
    ) -> list[NormalizedTransaction]:
        """
        ...
        for candidate in candidates:
            ...
            if self.retrieval_config.require_same_currency:
                if email.currency and candidate.currency and email.currency != candidate.currency:
                    logger.debug(
