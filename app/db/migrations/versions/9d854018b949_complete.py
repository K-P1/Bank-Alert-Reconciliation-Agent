...
def upgrade() -> None:
    ...
    sa.Column('confidence', sa.Numeric(precision=5, scale=4), nullable=True, comment='Alias for parsing_confidence (for compatibility)'),
    sa.Column('parsing_method', sa.String(length=50), nullable=True, comment='Parsing method used (llm, regex, hybrid)'),
    ...
    sa.Column('status', sa.String(length=50), nullable=False, comment="Match status: 'matched' (auto-accepted), 'review' (needs manual review), 'rejected' (below threshold), 'no_candidates' (no matches found), 'pending' (not yet processed)"),
    ...
    sa.Column('id', sa.Integer(), autoincrement=True, nullable=False),
    sa.Column('action_id', sa.String(length=100), nullable=False, comment='Unique identifier for this action'),
    sa.Column('action_type', sa.String(length=50), nullable=False, comment="Type of action (e.g., 'mark_verified', 'create_ticket')"),
    sa.Column('match_id', sa.Integer(), nullable=False, comment='Reference to the match record'),
    sa.Column('email_id', sa.Integer(), nullable=False, comment='Reference to the email record'),
    sa.Column('transaction_id', sa.Integer(), nullable=True, comment='Reference to the transaction record (if applicable)'),
    sa.Column('match_status', sa.String(length=50), nullable=False, comment='Match status at time of action'),
    sa.Column('match_confidence', sa.Numeric(precision=5, scale=4), nullable=False, comment='Match confidence at time of action'),
    sa.Column('match_outcome', sa.String(length=50), nullable=False, comment='Match outcome category (matched, ambiguous, unmatched, review)'),
    sa.Column('status', sa.String(length=50), nullable=False, comment='Action execution status (pending, success, failed, etc.)'),
    sa.Column('outcome', sa.String(length=100), nullable=False, comment='Brief outcome description'),
    sa.Column('message', sa.Text(), nullable=True, comment='Detailed message or error description'),
    sa.Column('error', sa.Text(), nullable=True, comment='Error message if action failed'),
    sa.Column('action_metadata', sa.Text(), nullable=True, comment='JSON blob with action-specific metadata'),
    sa.Column('action_payload', sa.Text(), nullable=True, comment='JSON blob with action payload (e.g., webhook body, ticket details)'),
    sa.Column('actor', sa.String(length=255), nullable=False, comment='Actor that triggered the action (system, user ID, service name)'),
    sa.Column('started_at', sa.DateTime(timezone=True), nullable=False, comment='When action execution started'),
    sa.Column('completed_at', sa.DateTime(timezone=True), nullable=True, comment='When action execution completed'),
    sa.Column('duration_ms', sa.Integer(), nullable=True, comment='Action execution duration in milliseconds'),
    sa.Column('retry_count', sa.Integer(), nullable=False, comment='Number of retry attempts'),
    sa.Column('is_retried', sa.Boolean(), nullable=False, comment='Whether this action was retried'),
    sa.Column('created_at', sa.DateTime(timezone=True), nullable=False),
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=False),
    sa.ForeignKeyConstraint(['email_id'], ['emails.id'], ondelete='CASCADE'),
    ...
    op.create_index('idx_action_audit_actor', 'action_audits', ['actor', 'started_at'], unique=False)
    op.create_index('idx_action_audit_match', 'action_audits', ['match_id', 'action_type'], unique=False)
    op.create_index('idx_action_audit_outcome', 'action_audits', ['match_outcome', 'action_type'], unique=False)
    op.create_index('idx_action_audit_status', 'action_audits', ['status', 'started_at'], unique=False)
    op.create_index(op.f('ix_action_audits_action_id'), 'action_audits', ['action_id'], unique=True)
    op.create_index(op.f('ix_action_audits_action_type'), 'action_audits', ['action_type'], unique=False)
    op.create_index(op.f('ix_action_audits_actor'), 'action_audits', ['actor'], unique=False)
    op.create_index(op.f('ix_action_audits_email_id'), 'action_audits', ['email_id'], unique=False)
    op.create_index(op.f('ix_action_audits_match_id'), 'action_audits', ['match_id'], unique=False)
    op.create_index(op.f('ix_action_audits_match_outcome'), 'action_audits', ['match_outcome'], unique=False)
    op.create_index(op.f('ix_action_audits_match_status'), 'action_audits', ['match_status'], unique=False)
    op.create_index(op.f('ix_action_audits_started_at'), 'action_audits', ['started_at'], unique=False)
    op.create_index(op.f('ix_action_audits_status'), 'action_audits', ['status'], unique=False)
    op.create_index(op.f('ix_action_audits_transaction_id'), 'action_audits', ['transaction_id'], unique=False)
