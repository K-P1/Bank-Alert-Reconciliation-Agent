...
class EnrichmentMetadata(BaseModel):
    ...
    bank_code: str | None = Field(
        default=None, description="Standardized bank code (e.g., GTB, FBN, ACC)"
    )
    bank_name: str | None = Field(
        default=None, description="Full bank name (e.g., Guaranty Trust Bank)"
    )
    channel: str | None = Field(
        default=None, description="Transaction channel (e.g., USSD, Mobile App, ATM)"
    )
    customer_id: str | None = Field(
        default=None, description="Extracted customer identifier"
    )
    ...
    enrichment_confidence: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Confidence in enrichment"
    )
...
class NormalizedReference(BaseModel):
    ...
    cleaned: str = Field(
        ..., description="Cleaned reference (stripped, normalized whitespace)"
    )
    tokens: list[str] = Field(
        default_factory=list, description="Extracted tokens for matching"
    )
    alphanumeric_only: str = Field(..., description="Only alphanumeric characters")
    normalized_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        description="When normalized",
    )
...
class CompositeKey(BaseModel):
    ...
    date_bucket: str = Field(
        ..., description="Date bucket for time window (YYYY-MM-DD-HH)"
    )
    reference_tokens: list[str] = Field(
        default_factory=list, description="Top reference tokens"
    )
...
class NormalizedEmail(BaseModel):
    ...
    amount: Decimal | None = Field(
        default=None, description="Normalized amount as Decimal"
    )
    currency: str | None = Field(
        default=None, description="ISO 4217 currency code (e.g., NGN, USD)"
    )
    ...
    reference: NormalizedReference | None = Field(
        default=None, description="Normalized reference data"
    )

    # Normalized timestamp (UTC)
    timestamp: datetime | None = Field(
        default=None, description="Normalized transaction timestamp (UTC)"
    )
    received_at: datetime = Field(..., description="When email was received (UTC)")

    # Enrichment data
    enrichment: EnrichmentMetadata | None = Field(
        default=None, description="Enrichment metadata"
    )

    # Composite key for matching
    composite_key: CompositeKey | None = Field(
        default=None, description="Composite key for matching"
    )

    # Original parsing metadata
    parsed_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        description="When email was parsed",
    )
    parsing_method: Literal["regex", "llm", "hybrid"] = Field(
        ..., description="Parsing method used"
    )
    parsing_confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Original parsing confidence"
    )
    ...
    normalization_quality: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Quality of normalization"
    )
